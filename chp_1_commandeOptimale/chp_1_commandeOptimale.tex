...
\begin{exercise}
	...
	\begin{enumerate}
		...
		\item Intégrer numériquement le système augmenté (système physique + observateur) montré dans la \cref{fig:LQG control scheme} sur 3 itérations avec $\deltaT= 0.01~s$, $\modelNoise^T(0) = \begin{bmatrix}
		-0.14 & 0.15
		\end{bmatrix}$,  $\modelNoise^T(\deltaT) = \begin{bmatrix}
		0.26 & 0.10
		\end{bmatrix}$, $\modelNoise^T(2\deltaT) = \begin{bmatrix}
		-0.15 & -0.07
		\end{bmatrix}$, et $\measureNoise(0) = -0.52$, $\measureNoise(\deltaT) = 1.1$, $\measureNoise(2\deltaT) = -2.10$.	
	\end{enumerate}
\end{exercise}