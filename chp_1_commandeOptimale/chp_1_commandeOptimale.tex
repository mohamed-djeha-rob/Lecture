...
\subsubsection{Solution du problème LQR à horizon fini}
...
En examinant la forme de la commande optimale $\command$ dans~\eqref{eq-chap2:forme commande optimale}, on remarque que 
\begin{itemize}
...
\section{Régulation de la sortie par la commande LQR}
...
\section{Cas d'un système nonlinéaire}
...
\begin{itemize}
	...
	\item La commande LQR est une commande par retour d'état. De ce fait, l'état $\state$ est supposé mesurable. Cependant, une mesure complète de l'état $\state$ est souvent cher, voire technologiquement impossible.
\end{itemize}
\section{Exercices}
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Résoudre symboliquement l'équation de Riccati.
		\item En posant $\alpha=1$, $\beta=0$, $\matriceQ = \begin{bmatrix}
			1 &0\\0&1
		\end{bmatrix}$ et $\matriceR=1$, résoudre numériquement l'équation de Riccati, et calculer la commande LQR à horizon infini qui stabilise le système linéaire obtenu.
